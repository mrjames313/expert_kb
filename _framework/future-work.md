...
**Partially shipped (2026-08-25):** `framework_check.py` now checks the *text* invariant — `framework_version` == the latest `UPGRADING.md` `**Release**` date (and releases ascending). What remains is the *git-history* half: "any pulled path changed since the commit that last bumped the version." That needs commit-range diffing, so it's the CI/pre-push piece proper.
...
**Hard-edge subset shipped (2026-08-25):** `framework_check.py` implements the three cheapest derived-vs-source edges — config `warnings_visible` == shipped warning rules, no pulled doc references a maintainer-only file, and version == latest release. It's wired into the release definition-of-done and guarded by a real-repo self-consistency test. What remains is the *general* graph: the git-history version-stamp edge (above), the soft prose edges (`SETUP.md` ↔ `role-template.md`), and the change-triggered worklist walk.
...
**Blocked on this (decided 2026-08-27):** the vitals cache deliberately caps its writers at three *structural* refresh points (`/kb-vitals`, `lint.py`, `/start` + the session-start hook) rather than having every mutating skill — `/promote`, `/exchange`, `/close-exchange`, `/propose-promotion` — refresh the counts it just invalidated. Targeted refreshes would make the indicator sharper, but an enumerated writer list is precisely this drift class: a new mutator lands, nobody adds the refresh, the indicator quietly goes stale. Add the extra writers **after** this graph can enforce the edge "a skill that changes a cached count must refresh the cache."
...
**Shipped (2026-08-26):** `statusline.py` + `statusline.sh` + the `statusLine` key in `.claude/settings.json`. Renders `<project> · <role@area | (no role)> · <⚠N | ✓> · ctx <N>k[!]` — role/area from this session's state file (`_session/<session-id>.json`, keyed on the payload's `session_id` since 2026-08-27), pending-human count from INBOX (cheap), context tokens via a tail-read (measured ~26ms on a 12MB transcript), `!` past the restart threshold. Verified live. **Indicator shipped (2026-08-27):** the single `⚠N | ✓` became **H** (human, project-wide) + **R** (role, adopted area), colored green/yellow/red by severity, with a `run /kb-vitals` hint when either is non-zero — backed by the hybrid cache below. Measured: 0.15ms → 0.57ms of scanning inside an unchanged ~26ms render (the process is almost entirely interpreter startup).

**Remaining:** show active capabilities in the line; a framework-vitals category (upgrade currency) once those exist; and extending the cache's writer set — see the note under the doc-dependency-graph entry.

**Original idea:** a Claude Code status line showing framework state — active capabilities, pending items, health — so you see it without running a command.
...
### Suggested-actions command — `/kb-vitals` (the "framework doctor")

**Named `/kb-vitals`** (2026-08-26). Organized as **human vitals** (project-wide, always shown — decisions/acks only the human can resolve: INBOX "Needs decision", promotions awaiting ack, proposals ready to promote) + **role vitals** (current area/role only, cheap checks — wrap-up due, pulse over-cap, restart-the-role suggestion, spec-complete→outcome, blocked tasks, exchange to close, stale preload) + **framework vitals** (deferred, its own category — upgrade currency).

**Foundation shipped (2026-08-26):** `session_state.py` + git-ignored per-session state (a single `_session.json` at first; sharded to `_session/<session-id>.json` on 2026-08-27) records adopted role/area, session id, transcript path, started_at; `/start` writes it, the session-start hook resets/stamps it, and a `transcript_tokens()` / `context_tokens()` helper reads the live context size from the session transcript (verified end-to-end). This gives `/kb-vitals` its role/area scope and turns the "long/bloated session" restart trigger into a real measurement. **Restart triggers to surface:** role/area switch, just-wrapped-up, long-session (context tokens past a configurable threshold — recommend *restart*, not `/clear`, per the hooks bug), stale preload (preload target `updated` > `started_at`). **Remaining:** the `/kb-vitals` command itself (the scan + presentation, next-actions-first) and its skill; a config `context_restart_threshold_tokens`; then the status-line indicator reads the same `_session.json`.

**Shipped (2026-08-26):** `kb_vitals.py` + the `/kb-vitals` skill implement human vitals (project-wide) + role vitals (current area, incl. context-bloat and stale-preload restart nudges); reads `_session.json`, runs no lint; verified live (fired the context-bloat vital on this ~596k-token session). Config `kb_vitals.context_restart_threshold_tokens` (default 400k). **Remaining:** framework vitals (upgrade currency) as its own category; the status-line indicator reading the same signals; and deferred cheap-but-fiddly checks (stalled `under_test` concepts, aging specs) once their lint rules exist.
