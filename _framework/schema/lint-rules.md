...
**Scope: kb pages, spec planning files, data manifests, and exchange files.** `/plan`, `/replan` and the brief template all tell agents to cite kb pages by wikilink from a `brief.md`, `plan.md`, `tasks.md`, `revisions.md` or `outcome.md`, so those files are checked on the same terms — a citation that resolves to nothing is the same defect wherever it is written. Specs are still exempt from *frontmatter* requirements (Rule 1 checks well-formedness only). Note that a reference from a spec to another non-kb file — another spec's `outcome.md`, a code file — is a relative markdown link, not a wikilink, so this rule never sees it.

Exchange files (`exchanges/*/ex-*.md`) carry a `## Context` section of area-prefixed wikilinks by protocol, and `/respond-exchange` has always told the responder that the wikilinks in their answer must resolve — a check that did not exist until the exchange iterator did. Their frontmatter is a different schema, so **Rule 22**, not Rule 1, validates it. The generated `index.md` is not walked (it is regenerated by Rule 15).
...
Forward links to pages with `status: superseded` are errors; the linter suggests the replacement via `superseded_by`. **Scope: kb pages, spec planning files, and exchange files** — a plan that proceeds from a retired decision, or an answer that hands one to another area, is the same defect as a page that cites one. The exchange case is the sharpest: the receiving area may preload a page on the strength of the answer. Area-prefixed links (`[[research:findings/f-…]]`) are resolved by stripping the prefix, so a cross-area citation is checked like any other.
...
The linter regenerates three kinds of index on every run, each derived wholly from files it reads elsewhere:

- `areas-index.md` — from area briefs and role summaries.
- each `kb/index.md` — from page frontmatter in that directory.
- each `exchanges/<a>--<b>/index.md` — from the pair's exchange frontmatter, grouped by status with open first. Entries are relative markdown links, since exchanges are not in the wikilink index. Not gated on `multi_area`: exchange directories survive disabling the capability, and an index that silently stopped tracking the files beside it would be worse than one kept honest.

A file is rewritten only when its generated content actually changed (the `_Last regenerated:_` stamp is masked out of the comparison), so a no-op run leaves the tree clean. Hand edits to any of these are overwritten rather than flagged — that is what the `L` maintenance category means.
...
Exchange files carry a schema of their own, sharing only three field names with a kb page — so Rule 1 cannot be pointed at them without emitting five spurious "missing required field" errors on every well-formed exchange. This is that rule. Per `exchange-protocol.md`:

- `kind` is `query` or `brief`.
- Both kinds require `id`, `kind`, `status`, `from_area`, `from_role`, `to_area`, `created`; a brief additionally requires `to_roles` and `open_for`. (`relevant_to` is a field-ordering convention, not a required field — kb pages don't require it either.)
- `status` is drawn from the vocabulary for its kind: `open | answered | follow_up | closed` for a query, `open | closed` for a brief, which has no answer cycle.
- A query carries no `to_roles`/`open_for`. The usual cause is a brief filed under the wrong kind, which routes it to `/respond-exchange` — where nobody owes it an answer — instead of to its targeted roles.
- `open_for ⊆ to_roles`, since it is drained from that snapshot; and a brief is `closed` exactly when `open_for` is empty. Both are asserted by the protocol and were checked nowhere.
- `id` matches `ex-YYYY-MM-DD-<slug>` and agrees with the filename; `created` is an ISO date.
- The parties differ, and the file sits in the canonical directory for the pair (areas sorted, joined by `--`, a sub-area's slash flattened to `-`). This last one keeps an exchange *findable*: every consumer globs `exchanges/*/`, so a misfiled exchange is not an error anywhere — it simply never surfaces to the role that owes it a response.
