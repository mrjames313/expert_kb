...
### Rule 2's relative-markdown-link clause is documented but unimplemented

`lint-rules.md` has said, under Rule 2, "Every relative markdown link to a repo path resolves to an existing file" since the rule was written. `rule_02_forward_links.py` checks wikilinks (body + frontmatter) and `provenance.raw_path` — nothing else. So the *other* half of the framework's link story is unenforced: `link-conventions.md` directs every reference to a non-kb file (code, manifests, other specs, raw materials, exchange files) to be a relative markdown link, and none of those are ever resolved.

Surfaced 2026-08-30 while extending Rules 2 and 5 to spec files. It matters more now: `/plan` was just corrected to link a prior spec's `outcome.md` relatively rather than by wikilink, which moves that citation from a checked form into an unchecked one. Marked as unimplemented in `lint-rules.md` rather than silently left.

**What addressing it would look like:** extract `[text](path)` links from bodies, skip external URLs, anchors, and code fences (`extract_wikilinks` has no fence handling either — same helper gap), resolve the rest relative to the containing file, and error on a miss. The fence-skipping is the fiddly part; `rewrite_links_to_twins` in `common.py` already has a working implementation to lift.

**Revisit when:** a broken relative link causes a real miss — most likely a spec→spec `outcome.md` reference now that `/plan` produces them, or a manifest pointing at a moved data file. This is the eighth "documented behaviour, no implementation" instance; per `maintaining.md` the clause should have had an enforcer or an explicit advisory marker from the start.
