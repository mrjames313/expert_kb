...
3. Set frontmatter:
   ...
   - `aligned_on: <date>` (last reconciled with the source; drives drift detection)
   - drops `relevant_to` (commons is relevant to everyone)
4. Edit the new commons page for a commons reader — a full copy, not a summary: strip resolved-deliberation cruft (resolved-question sections, superseded sections, closed "left open" lists) and rewrite conceptual links to their commons twins (leave provenance links to area sources; see `commons_links.py`).
5. Leave the source area page's content and status unchanged, but add a `commons_twin: "[[<new-commons-id>]]"` back-pointer to it — the one sanctioned cross-area metadata write, and what makes drift detection bidirectional. Do **not** mark the source `superseded` — superseding is for a page that has been *replaced*, and linking to a superseded page is a lint error (see `link-conventions.md`), so it would turn every inbound citation to the area page into an error.
6. Write a `CHANGELOG.md` entry (citing the new commons id).
7. File an INBOX entry under "Awaiting your ack": "Promoted [[finding]] — awaiting human review."
