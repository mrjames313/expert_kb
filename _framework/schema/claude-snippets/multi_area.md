...
- **Don't write** into another area's kb or role files — file an `/exchange` instead. This is a convention, not a checked boundary; a genuine exception belongs in conversation with a visible trace, not a silent write. The one exception is the `commons_twin` back-pointer set during promotion (framework-maintained metadata).
